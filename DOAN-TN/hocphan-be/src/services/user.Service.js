...
module.exports.deleteUser= async (userId) => {
    ...
    await CartDtb.deleteOne({ user: userId });
    
    // Xóa favorite của user
    const FavoriteDtb = require('../models/Favorite.Model');
    await FavoriteDtb.deleteMany({ user: userId });
...
module.exports.deleteManyUser=async(ids) =>{
    // Xóa cart của các user
    const CartDtb = require('../models/Cart.Model');
    await CartDtb.deleteMany({ user: { $in: ids } });
    
    // Xóa favorite của các user
    const FavoriteDtb = require('../models/Favorite.Model');
    await FavoriteDtb.deleteMany({ user: { $in: ids } });
