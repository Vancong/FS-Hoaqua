...
const CardComponent = (props) => {
  const { description, images = [], name, slug, product, state, width = null } = props;
  ...
  const handleDetailProduct = () => {
    const targetSlug = product?._id || props._id || slug;
    navigate(`/product-details/${targetSlug}`, { state: { category: state, product: name, slugCt: location.pathname } })
  }
  ...
  const rawImage = product?.image || (images && images[0] ? images[0] : '');
  const productImage = getFruitImage(name || product?.name, rawImage);
  ...
  const handlAddCart = (type = "") => {
    ...
    if (productStock === 0 || productStock < cartQty + 1) {
      alertError(`Chỉ còn ${productStock} kg trong kho`);
      return false;
    }
  ...
  return (
    <>
      ...
      >
        {discount > 0 && (<p className='discount-product'>{`-${discount}%`}</p>)}
        ...
        <div className='info_bottom' onClick={handleDetailProduct}>
            ...
            {discount > 0 ? (
              <>
                <span className="new_price">
                  {Fomater(getDiscountPrice(productPrice, discount))}
                </span>
                <span className="old_price">{Fomater(productPrice)}</span>
                <span className="unit_hint"> / kg</span>
              </>
            ) : (
              <>
                {Fomater(productPrice)}
                <span className="unit_hint"> / kg</span>
              </>
            )}
          </div>
          <p className="stock_hint">Còn {productStock} kg</p>
        </div>
        ...
        <div className='product_order'>
          ...
          <div className='icon_detail' onClick={handleDetailProduct}>
            <EyeOutlined />
          </div>
        </div>
      </Card>
